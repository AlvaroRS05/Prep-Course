...
function agregarNumeros(numeros) {
  ...
  // Tu código:
  return numeros.reduce((total, currentValue) => total + currentValue);
}
...
function numeroMasGrande(numeros) {
  ...
  numeros.forEach((item) => {
    numeroGrande = item > numeroGrande ? item : numeroGrande;
  });
  return numeroGrande;
}
...
function multiplicarArgumentos() {
  ...
  let copiaArray = [...arguments];
  let r = 0;
  if (arguments.length === 0) return 0;
  if (arguments.length === 1) return arguments[0];

  return copiaArray.reduce((acc, current) => acc * current);
}
...
function todosIguales(arreglo) {
  ...
  return r === arreglo.length - 1 ? true : false;
}
...
function mayorACien(array) {
  ...
  // Tu código:
  return array.filter((item) => (item > 100 ? item : ""));
}
...
function breakStatement(numero) {
  ...
  return i === 10 ? nuevoArray : "Se interrumpió la ejecución"
